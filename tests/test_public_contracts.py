...
def test_v1_gate_includes_real_runtime_flow(tmp_path) -> None:
    ...
    assert "mcp_authoring_smoke" in names
    ...
    assert "generated_line_scenario_enrich" in names
    assert "generated_waypoint_scenario_enrich" in names
    assert "generated_obstacle_scenario_enrich" in names
    ...
    assert "generated_waypoint_world_edit" in names
    assert "imported_world_edit" in names
